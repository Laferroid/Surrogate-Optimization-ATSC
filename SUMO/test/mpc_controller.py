...
class MPCController:
    def __init__(self,model,mpc_config):
        ...
        self.phase_list = []
        for i in range(2**5):
            self.phase_list.append(np.array([(i//2**4)%2, (i//2**3)%2, (i//2**2)%2, (i//2**1)%2, (i//2**0)%2]))
    ...
    def _optimize_d(self,phases,splits):
        ...
        for i in range(len(phases)):
            ...
            for j in range(len(splits)):
                ...
                linear_constraint = LinearConstraint(np.vstack((A1,A2,A3)),np.concatenate([lb1,lb2]),np.concatenate([ub1,ub2]),keep_feasible=True)
                res = minimize(fun=objective_func,x0=split,method='trust-constr',jac=True,bounds=bounds,constraints=linear_constraint,
    ...
    def _optimize(self,phases,splits):
        ...
        def par_func(j):
            ...
            lb1,ub1 = self.G_min*np.ones(lookahead),self.G_max*np.ones(lookahead)
            lb2,ub2 = np.zeros(2*lookahead),np.zeros(2*lookahead)
            linear_constraint = LinearConstraint(np.vstack((A1,A2,A3)),np.concatenate([lb1,lb2]),np.concatenate([ub1,ub2]),
                                                 keep_feasible=False)
            res = minimize(fun=objective_func,x0=split,method='trust-constr',jac=False,
    ...
    def _objective_func(self,split,phase):
        ...
        tc_ahead = torch.from_numpy(np.concatenate([phase,split],axis=-1)).to(torch.float32).unsqueeze(dim=0)  # (1,lookahead,13)
    ...
    def update(self,recorder,tc):
        ...
        # tc_back更新
        control = torch.from_numpy(np.concatenate([tc.control['phase'],tc.control['split']]))
