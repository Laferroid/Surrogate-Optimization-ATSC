...
res = Parallel(n_jobs=8,verbose=10)(delayed(par_func)(controls[i//num,i%num]) for i in range(num*num))

with open(output_dir+f"res_grid_{step_ahead}.npy",'wb') as f:
    np.save(f,np.array(res).reshape((num,num)))