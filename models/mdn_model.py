...
class MixtureDensityNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_componets):
        ...
        self.sigma_layer = nn.Sequential(
            nn.Linear(self.hidden_size, self.num_componets * self.output_size), SigmaLayer()
        )
    ...
    def loss_func(self, y_pred, y):
        ...
        c_log_prob_1 = (
            -(self.output_size / 2)
            * torch.log(torch.tensor(2 * math.pi, device=device))
            * torch.ones_like(c, device=device)
        )
...
def prediction_interval(y_pred, z):
    c, mu, sigma = y_pred
    def cdf(x):
        return (c * norm(loc=mu, scale=sigma).cdf(np.expand_dims(x, 0))).sum(0)
