...
class UncertaintySurrogateModel(nn.Module):
    def __init__(self,config):
        ...
        self.conv = nn.Sequential(nn.Conv2d(20,self.num_channels,kernel_size=(7,3),padding=(3,1)),
        ...
        self.dec_layer = nn.LSTMCell(input_size=hidden_size+1,hidden_size=hidden_size)  # because of recursive feeding
        # output delay
    ...
    def _decoding(self,upper_context,tc_ahead):
        ...
        if self.config['train_predict_mode'] == 'recursive':  # 将mdn输出的mean进行feed forward
            ...
            for i in range(tc_ahead.shape[1]):
                (h_state,c_state) = self.dec_layer(torch.cat([tc_ahead[:,i,:],out],dim=-1),(h_state,c_state))  # dec_output: (batch,*)
            ...
            for i in range(tc_ahead.shape[1]):
                (h_state,c_state) = self.dec_layer(torch.cat([tc_ahead[:,i,:],z],dim=-1),(h_state,c_state))  # dec_output: (batch,*)
